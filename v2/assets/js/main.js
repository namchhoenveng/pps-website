/* ==========================================================================
   Paris Partners Softwares — v2 behaviour
   ---------------------------------------------------------------------------
   Deliberately small. v2 has no scroll-reveal, no animated counters and no
   client-side filtering, so there is nothing here that content depends on:
   with JavaScript disabled the site loses a theme toggle and a mobile drawer,
...
(function () {
  ...
  /* ----------------------------------------------------------------- theme */
  /* The inline snippet in <head> applies the stored theme before first paint.
  ...
  function initTheme() {
    ...
    toggle.addEventListener("click", function () {
      ...
      try {
        localStorage.setItem("pps-theme-v2", next);
      } catch (e) {
  ...
  function initNav() {
    ...
    var wide = window.matchMedia("(min-width: 61.0625rem)");
  ...
  /* ------------------------------------------------------------------ form */
  /* Posts JSON to contact.php, exactly like v1. Without a data-endpoint the
     form validates and then points the visitor at the e-mail address rather
